...
test('Case 16: 繁体「後」与多余杂质行混排记谱解析 (後炮平七 / 後仕進五 / Classification icon)', () => {
  const text = `
1
兵三進一
卒3進1
...
`;
  ...
  const res = importXiangqiGame(text);
  assert.equal(res.success, true, res.error);
  assert.equal(res.moves.length, 85, 'Should parse all 85 moves');
  assert.equal(res.chineseMoves[40], '后炮平七');
  assert.equal(res.chineseMoves[60], '后仕进五');
  assert.equal(res.chineseMoves[84], '炮九退二');
});
